...
DatosMinSalTodo<-read.csv("CasosEntreRios.csv")
## SELECCIONO SOLO LOS CONFIRMADOS
DatosMinSal<-DatosMinSalTodo %>% filter(clasificacion_resumen =="Confirmado")


#####
localidades<-c("Entre Ríos","Paraná","Concordia","Gualeguaychú")
Poblocalidades<-c(1385961,376912,191117,124231) #estimado indec al 2020
...
if(is.na(nombresparaarchivo[[1]][2])){
  ...
  write.csv(riesgoTODAS[[j]],paste("Riesgo",nombreprovinciasinacento,nombresparaarchivo[[1]][2],sep=""),row.names = FALSE)
}
}
