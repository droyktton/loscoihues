...
DatosMinSalTodo<-read.csv("CasosSalta.csv")
## SELECCIONO SOLO LOS CONFIRMADOS
DatosMinSal<-DatosMinSalTodo %>% filter(clasificacion_resumen =="Confirmado")
#####
localidades<-c("Salta","Capital","General Güemes","Orán","Grl. José de San Martín")
Poblocalidades<-c(1424397,631058,55211,162562,184877) #estimado indec al 2020
...
posCoordsVec2 <- c(ymd("2020-04-25"),ymd("2020-06-15"))
...
if(is.na(nombresparaarchivo[[1]][2])){
  write.csv(riesgoTODAS[[j]],paste("Riesgo",localidades[1],nombresparaarchivo[[1]][1],sep=""),row.names = FALSE)  
}else{
  write.csv(riesgoTODAS[[j]],paste("Riesgo",localidades[1],nombresparaarchivo[[1]][2],sep=""),row.names = FALSE)
}
}
