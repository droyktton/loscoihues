...
DatosMinSalTodo<-read.csv("CasosTierraDelFuego.csv")
## SELECCIONO SOLO LOS CONFIRMADOS
DatosMinSal<-DatosMinSalTodo %>% filter(clasificacion_resumen =="Confirmado")
#####Rio Gallegos esta en GuerAike, Calafate está en Lago Argentino 
localidades<-c("Tierra del Fuego","Río Grande","Ushuaia")
Poblocalidades<-c(173432,95889,77260) #estimado indec al 2020
...
nombreprovinciasinacento<-gsub(" ", "",localidades[1] , fixed = TRUE)
...
for(i in 1:length(DatosMinSal$fecha_inicio_sintomas))
{
  ...
  {
    diasentrefechas<-ceiling(runif(1,min=0,max=8)) ###los limites salen de un ajuste de Ale a datos reales.
...
posCoordsVec2 <- c(ymd("2020-04-15"),ymd("2020-07-01"))
...
write.csv(outprint,paste("NuevosCasosConfirmadosPorDia",nombreprovinciasinacento,".csv",sep=""),row.names = FALSE)
...
nombresparaarchivo<-strsplit(stri_trans_general(localidades[j],"Latin-ASCII")," ")
...
if(is.na(nombresparaarchivo[[1]][2])){
  ...
  write.csv(riesgoTODAS[[j]],paste("Riesgo",nombreprovinciasinacento,nombresparaarchivo[[1]][1],nombresparaarchivo[[1]][2],sep=""),row.names = FALSE)
}
}
