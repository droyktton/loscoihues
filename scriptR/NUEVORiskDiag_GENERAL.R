...
#####
localidades<-c("CABA","COMUNA 1","COMUNA 3","COMUNA 4","COMUNA 7","COMUNA 8")
...
#considero fecha inicio de sintomas y si no hay le pongo la fecha de ingreso al SISA
...
DatosMinSal$fechcompleta<-colfechas
...
##################################################################
#### AJUSTE DE LOS CASOS ACUMULADOS CON GOMPERTZ 
# yy_=0
# xx_=0
# # Function to fit
...
# }
# 
# #Datos a ajustar con Gompertz
# data_model = data.frame(xx_ = index(casos_consecutivos[[1]]) , yy_ = casos_consecutivos[[1]]$casos_diarios, time = seq(0, length(xx_)-1,1))
# 
...
# }
# 
# #Ajuste de casos acumulados con Gompertz
# fitting = nls(
...
#propagate es para progpagar errores
#library(propagate)
...
#casos_acumulados<-cumsum(casos_consecutivos[[1]]$casos_diarios)
#newcases<-casos_consecutivos[[1]]$casos_diarios
#fechas<-index(casos_consecutivos[[1]])
#rhoprueba<-calculo_rho_extrap(newcases,fechas,casos_acumulados)

#riesgoprueba<-calcmeanRho7yA14_extrap(rhoprueba$rhodays,rhoprueba$nuevasfechas,casos_consecutivos[[1]]$casos_diarios,index(casos_consecutivos[[1]]),Poblocalidades[1])
...
if(is.na(nombresparaarchivo[[1]][2])){
  write.csv(riesgoTODAS[[j]][-nrow(riesgoTODAS[[j]]),],paste("Riesgo",localidades[1],nombresparaarchivo[[1]][1],sep=""),row.names = FALSE)  
}else{
write.csv(riesgoTODAS[[j]][-nrow(riesgoTODAS[[j]]),],paste("Riesgo",localidades[1],nombresparaarchivo[[1]][2],sep=""),row.names = FALSE)
}
  }
